...
public class ImageDisplayController : MonoBehaviour, IPointerClickHandler, IPointerMoveHandler
{
    ...
    // True only for dynamically Instantiate()'d copies created via
    // Image_UI_Manager.VR_Arrastar (the player can drag these out into the
    // world and leave them open independently of the main gallery panel).
    // The persistent main-panel placeholders (placeholder1/2, fullscreen)
    // reuse this same component but are never destroyed during normal play,
    // so this flag keeps OnDestroy() from releasing a reference they never
    // took out in the first place.
    public bool isDynamicWorldCopy = false;
    ...
    void OnDestroy()
    {
        // Only dragged-out world copies took out their own reference - the
        // persistent placeholders never call AddHotspotReference, so they
        // must not release one here.
        if (isDynamicWorldCopy && HotspotManager.Instance != null)
            HotspotManager.Instance.ReleaseHotspotReference(currentHotspotId);
    }
    ...
    {
        TelemetryLogger.Instance.LogUIInteraction("Create Note");
    ...
    {
        TelemetryLogger.Instance.LogUIInteraction("Edit Note");
        annotationManager.EditNote(note, this, onConfirm);
    }

    public void DeleteNote(NoteData data)
    {
        TelemetryLogger.Instance.LogUIInteraction("Delete Note");
